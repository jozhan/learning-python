    ...
    for filename in listdir(image_dir):
        if filename not in results_dic:
            ...
            for word in filename_word_list:
                if word.isalpha():
                    # Only append word when it is not alphabetic character
                    pet_label += word + " "
            results_dic[filename] = [pet_label.strip()]
        else:
            # Print warning if filename exists in the dictionary already
            print("** [Warning][get_pet_labels]: Key={} already exists in the results_dic with value={}",
                  filename,
